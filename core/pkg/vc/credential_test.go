...
func TestIssueVerifySubjectCredentialRoundtrip(t *testing.T) {
	issKP, issuer, kid := issuerSetup(t)
	_, subject := subjectSetup(t)

	now := time.Date(2026, 5, 1, 0, 0, 0, 0, time.UTC)
	in := SubjectCredential{
		Issuer:      issuer,
		Subject:     subject,
	...
	jwt, err := IssueSubjectCredential(issKP, in, IssueOptions{IssuerKeyID: kid})
	if err != nil {
		t.Fatalf("IssueSubjectCredential: %v", err)
	}

	out, err := VerifySubjectCredential(context.Background(), did.NewKeyResolver(), jwt, now)
	if err != nil {
...
func TestIssueRejectsLifetimeOverCap(t *testing.T) {
	...
	if _, err := IssueSubjectCredential(issKP, in, IssueOptions{IssuerKeyID: kid}); err == nil {
...
func TestVerifyExpiredSubjectCredential(t *testing.T) {
	issKP, issuer, kid := issuerSetup(t)
	_, subject := subjectSetup(t)
	...
	_, err = VerifySubjectCredential(context.Background(), did.NewKeyResolver(), jwt, iat.Add(2*time.Hour))
	if err == nil {
...
func TestVerifyTamperedSignature(t *testing.T) {
	...
	jwt, _ := IssueSubjectCredential(issKP, in, IssueOptions{IssuerKeyID: kid})
	...
	if _, err := VerifySubjectCredential(context.Background(), did.NewKeyResolver(), tampered, now); err == nil {
