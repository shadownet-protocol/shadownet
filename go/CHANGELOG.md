...
`go/pgstore/vX.Y.Z` to satisfy Go's directory-prefix tag requirement for
sub-module subtrees. The main module is tagged `go/vX.Y.Z`.
...
- **BREAKING:** the project moved from
  ...
  monorepo at `go/`. The Go module path is now
  `github.com/shadownet-protocol/shadownet/go`; the `pgstore` submodule
  path is now `github.com/shadownet-protocol/shadownet/go/pgstore`.
