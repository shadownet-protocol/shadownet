...
def test_missing_rfc_marker_in_required_dir_blocks_collection(pytester: pytest.Pytester):
    ...
    pytester.mkdir("tests/issuer")
    test_path = pytester.path / "tests" / "issuer" / "test_missing_marker.py"
    ...
    assert "issuer/test_missing_marker.py" in combined
...
def test_present_rfc_marker_in_required_dir_collects(pytester: pytest.Pytester):
    ...
    pytester.mkdir("tests")
    pytester.mkdir("tests/issuer")
    test_path = pytester.path / "tests" / "issuer" / "test_with_marker.py"
    test_path.write_text(
        (
            "import pytest\n"
            "@pytest.mark.rfc('0001', section='6.5', requirement='Issuance')\n"
            "def test_present():\n    assert True\n"
        ),
        encoding="utf-8",
    )
